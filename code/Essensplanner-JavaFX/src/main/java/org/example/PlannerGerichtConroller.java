...
public class PlannerGerichtConroller {
    ...
    private void initialize() {
        ...
        Pane trashPane = new Pane();
        Pane rezeptPane = new Pane();

        rezeptPane.setPrefWidth(Region.USE_COMPUTED_SIZE);


        rezept.setFill(Paint.valueOf("#ffffff"));
        rezept.setOnMouseClicked(new EventHandler<MouseEvent>() {
            @Override
        ...
        addSvg(new SVGPath[]{rezept});
    }

    private void addSvg(SVGPath[] svgPaths){
    ...
    private void setPercentWidth(){
        double perWidth = iconbox.getChildren().size() * 10;
        labelbox.setPercentWidth(100 - perWidth);
